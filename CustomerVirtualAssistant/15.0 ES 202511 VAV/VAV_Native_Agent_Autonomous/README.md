...
* Select Webex AI Agent as Autonomous in the VirtualAgentVoice element to configure the Agent ID required by Webex AI Agent.

* Provide a valid Agent ID in the element settings, as this is mandatory for initiating the AI session.

* VAV handles the internal conversational loop, streams user audio to the Webex AI Agent, receives responses, plays them back, and continues until an exit condition is provided by the agent.
...
* For Webex AI Agent flow: In the VirtualAgentVoice element, mention the Agent ID in the "Agent ID" field. This is a mandatory field.
...
* The file attached contains:
  * VAV_Native_Agent_Autonomous -  AI Agent with Fallback as well as agent transfer and end session.
